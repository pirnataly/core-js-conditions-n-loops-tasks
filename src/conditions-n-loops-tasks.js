...
function convertToRomanNumerals(num) {
  ...
  let first;
  let second;
  const decimalDigit = Math.trunc(num / 10);
  switch (decimalDigit) {
    case 1:
      first = 'X';
      break;
 ...
 * Take int o account that the matrix size can be very large. Consider how you can optimize your solution.
...
function sortByAsc(arr) {
  function sortFunction(arr1) {
    ...
    return [...sortFunction(a), p, ...sortFunction(b)];
  }

  const newarray = arr;
  const array = sortFunction(newarray);
  newarray.length = 0;
...
function shuffleChar(str, iterations) {
  ...
  while (ind < iterations) {
    ...
    ind += 1;
    if (newstr === str) {
      const repeatAmout = Math.trunc(iterations / ind);
      ind *= repeatAmout;
    }
  }
  return newstring;
